 ...
 * recorded only by the progress write cores, which write the DONE PROGRESS CareEvent directly.
 *
 * Exported so the owner path can fail fast BEFORE opening a transaction — one definition, two call
 * sites, never a second copy of the rule. The core calls it too, so the applier is guarded regardless.
 */
export function assertNotProgressTask(task: Task): void {
  if (task === 'PROGRESS') throw new BadRequestException('PROGRESS is not a valid feedback task');
}
...
export type RecordFeedbackInput = {
  ...
  task: Task;
  type: CareEventType;
  /** Native Date. Never bind an ISO string into a raw date comparison (MariaDB tz hazard). */
  occurredOn: Date;
  postponeToOn?: Date;
  /** Top-level WATER feedback reason (spec B §4) — persisted into CareEvent.payload. */
  reason?: string;
  payload?: unknown;
  audit: AuditContext;
};
...
/**
 * The single implementation of "record a care feedback event". Called by FeedbackService.record (owner
 * path) and by the proposal applier — only the AuditContext differs.
 *
 * Before this refactor these were 3–5 sequential UNTRANSACTED writes: an event could be created and the
 * override left un-deleted if the process died between them. The caller now supplies one transaction, so
 * the whole feedback is atomic.
 */
export async function recordFeedbackCore(
  tx: Prisma.TransactionClient,
  input: RecordFeedbackInput,
): Promise<CoreResult<{ ok: true }>> {
  ...
  // adaptation, so reject any plant the actor may not touch before mutating.
  ...
  if (input.task === 'REPOT') {
    await recordRepotFeedback(tx, input);
    await auditFeedback(tx, input);
  ...
  if (input.type === 'DONE' && input.task === 'WATER') {
    ...
    const previousAnchor =
      previous?.occurredOn ??
      (
        await tx.plant.findUniqueOrThrow({
          where: { id: input.plantId },
          select: { acquiredOn: true },
        })
      ).acquiredOn;
    const dueRow = await tx.dueCache.findUnique({
      where: { plantId_task: { plantId: input.plantId, task: 'WATER' } },
      select: { nextDueOn: true },
    });
    const hadOverride =
      (await tx.taskOverride.count({ where: { plantId: input.plantId, task: 'WATER' } })) > 0;
  ...
  await tx.careEvent.create({
    data: {
      ...
      type: input.type,
      occurredOn: input.occurredOn,
      ...(mergedPayload === undefined ? {} : { payload: mergedPayload as Prisma.InputJsonValue }),
    },
  });

  if (input.type === 'DONE') {
    // (4) delete the override (existing DONE behaviour).
    await tx.taskOverride.deleteMany({ where: { plantId: input.plantId, task: input.task } });
  ...
  if (input.type === 'POSTPONED' && input.postponeToOn) {
    ...
    if (input.task !== 'WATER') await adapt(tx, input.plantId, input.task);
  }
...
/** One audit row per recorded feedback, in the same transaction as the writes it describes. */
async function auditFeedback(tx: Prisma.TransactionClient, input: RecordFeedbackInput): Promise<void> {
  await writeOriginAudit(tx, {
    ...
    operationType: input.type === 'DONE' ? 'care.done' : `care.${String(input.type).toLowerCase()}`,
    targetTable: 'care_events',
    targetId: null,
    payload: {
      task: input.task,
      type: input.type,
...
async function snapshotRepotObservation(tx: Prisma.TransactionClient, plantId: string, now: Date) {
  const [profile, sized] = await Promise.all([
    tx.plantProfile.findUnique({
      where: { plantId },
      select: { potSizeCm: true, growthHabit: true },
    }),
    latestSizedHeight(tx, plantId, now),
  ]);
...
async function recordRepotFeedback(
  tx: Prisma.TransactionClient,
  input: { plantId: string; type: CareEventType; occurredOn: Date; reason?: string; payload?: unknown },
): Promise<void> {
  ...
  const now = new Date();
  const snap = await snapshotRepotObservation(tx, input.plantId, now);
  const basePayload = {
    ...(input.payload as Record<string, unknown> | undefined),
    R_obs: snap.rObs,
  ...
  if (input.type === 'DONE') {
    ...
    await tx.taskOverride.deleteMany({ where: { plantId: input.plantId, task: 'REPOT' } });
    return;
  }
  ...
  if (routedTo === 'adjustment' && justified) {
    const current =
      (
        await tx.plantTaskAdjustment.findUnique({
          where: { plantId_task: { plantId: input.plantId, task: 'REPOT' } },
        })
      )?.multiplier ?? 1;
    const multiplier = nextRepotAdjustment(current, reason);
    await tx.plantTaskAdjustment.upsert({
      where: { plantId_task: { plantId: input.plantId, task: 'REPOT' } },
...
// The generic, un-gated adaptation. REPOT no longer reaches it (see `recordFeedbackCore`). It now serves
// only FERTILIZE / ROTATE / CLEAN_LEAVES / MIST, which still carry the F1.2 defect — named, scheduled
// separately, and deliberately out of Spec F's scope.
async function adapt(tx: Prisma.TransactionClient, plantId: string, task: Task): Promise<void> {
  const since = new Date(Date.now() - POSTPONE_WINDOW_DAYS * 86_400_000);
  const recentPostpones = await tx.careEvent.count({
